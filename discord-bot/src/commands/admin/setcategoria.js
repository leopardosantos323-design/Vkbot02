...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('[ADMIN] Definir a categoria de um produto')
    .addStringOption(o => o.setName('produto').setDescription('Nome do produto').setRequired(true))
    .addStringOption(o => o.setName('categoria').setDescription('Nova categoria').setRequired(true)),
  async execute(interaction) {
    ...
    const ok = db.updateProduto(nome, { categoria });

    if (!ok) return interaction.reply({ content: `❌ Produto **${nome}** não encontrado.`, ephemeral: true });
    await interaction.reply({ content: `✅ Categoria de **${nome}** definida como **${categoria}**` });
  },
};
